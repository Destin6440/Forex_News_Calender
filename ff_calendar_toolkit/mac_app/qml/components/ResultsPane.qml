...
Rectangle {
    ...
    ColumnLayout { anchors.fill:parent;anchors.margins:DesignTokens.section;spacing:DesignTokens.spaceLarge
        ...
        TabBar{id:views;objectName:"resultsViewSwitch";Layout.fillWidth:true;currentIndex:controller.resultsView;Accessible.name:"Results view";onCurrentIndexChanged:controller.saveResultsView(currentIndex);TabButton{text:"List";Accessible.name:"List view"}TabButton{text:"Calendar";Accessible.name:"Calendar view"}}
        StackLayout{Layout.fillWidth:true;Layout.preferredHeight:220;currentIndex:views.currentIndex
            ...
            GridView{id:calendar;model:resultModel;cellWidth:112;cellHeight:68;clip:true;activeFocusOnTab:true;Accessible.name:"Matching dates calendar";delegate:Button{required property string date;required property int matchedCount;width:104;height:60;text:date.slice(5)+"\n"+matchedCount+" matched";Accessible.name:date+", "+matchedCount+" matched events";onClicked:controller.selectDate(date)}}
        }
